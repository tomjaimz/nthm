...
const readSettings = async () => {
  ...
  document.getElementById("clientId").onchange = async () =>
    saveSettings("id", "clientId");
  document.getElementById("clientSecret").onchange = async () =>
    saveSettings("secret", "clientSecret");
  document.getElementById("redirectURI").onchange = async () =>
    saveSettings("redirect_uri", "redirectURI");

  document.getElementById("saveClientId").onclick = async () =>
    saveSettings("id", "clientId");
  document.getElementById("saveClientSecret").onclick = async () =>
    saveSettings("secret", "clientSecret");
  document.getElementById("saveRedirectURI").onclick = async () =>
    saveSettings("redirect_uri", "redirectURI");
...
window.onSpotifyWebPlaybackSDKReady = async () => {
  ...
  const requestToken = async (body) => {
    ...
    if (!token.refresh_token && body.refresh_token)
  ...
  if (searchParams.get("code")) {
    ...
    location.href = `${pathname}?request_result=${
      redirect_uri_invalid ? "invalid_redirect_uri" : "success"
    }`;
    return;
  }

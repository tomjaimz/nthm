...
const JSON_FORMATTER_URL = '//cdn.jsdelivr.net/npm/json-formatter-js/dist/json-formatter.umd.min.js'

const FAVICON_BASE64 = ""

/* Settings passed to client */
const PLAYER_NAME = 'Web Playback SDK [' + new Date().toISOString() + ']'
const ACTIONS = ['newToken', 'refreshToken', 'me', 'playerState', 'connect', 'disconnect', 'transfer', 'getCurrentState', 'getVolume', 'setVolume#0', 'setVolume#0.5', 'setVolume#1', 'pause', 'resume', 'togglePlay', 'previousTrack', 'nextTrack', 'seek#0', 'seek#10000']
const EVENTS = ['initialization_error', 'authentication_error', 'account_error', 'playback_error', 'player_state_changed', 'ready', 'not_ready']
...
const getFile = filename => fs.existsSync(filename) && fs.readFileSync(filename)
...
<html>
  ...
  ${getFile(BODY_HTML) || ''}
    <script>
const { PLAYER_NAME, ACTIONS, EVENTS } = ${JSON.stringify({ PLAYER_NAME, ACTIONS, EVENTS })}
    </script>
...
const getAuthorizeURL = () => {
    for (const state in verifications) {
        console.log(`${state} ${verifications[state].expiry - new Date().getTime()}`);
    ...
    verifications[state] = { codeVerifier, codeChallenge, expiry: new Date().getTime() + 10 * 60 * 1000 }
    ...
    http.createServer(async (req, res) => {
        try {
                ...
                req.on('data', c => { body += c }).on('end', async () => {
                    try {
                        ...
                        if (data.grant_type === 'authorization_code') {
                            const { code, state, grant_type, redirect_uri } = data
                            const code_verifier = state ?? verifications[state]?.codeVerifier

                            params = new URLSearchParams({ code, grant_type, redirect_uri, code_verifier });
                        }
                        if (data.grant_type === 'refresh_token') {
                            const { refresh_token, grant_type } = JSON.parse(body)
                            params = new URLSearchParams({ refresh_token, grant_type });
                        }
                        console.log({
                            method: 'POST',
                            body: params,
                            headers: {
                                Authorization: `Basic ${Buffer.from(`${settings.client.id}:${settings.client.secret}`).toString('base64')}`,
                            }
                        })
                        ...
                        if (response.ok) {
                            ...
                            if (token.refresh_token) {
                                console.log(token.refresh_token ? "Token aquired" : "Token refreshed")
                            }
                            ...
                            res.writeHead(response.status, { 'Content-Type': 'application/json' })
                                .end(JSON.stringify(await response.text()))
                        }
                    }
                    catch (e) {
                        console.log({ e })
                        ...
                        if (JSON.parse(body).code !== "preflight") {
                            console.log('Authorization error.')
                            if (e.body.error == 'invalid_grant') {
                                if (e.body.error_description == 'Invalid authorization code') {
                                    console.log(`code paramater incorrect`)
                                }
                                if (e.body.error_description == 'Invalid redirect URI') {
                                    console.log(`Redirect URI ${settings.client.redirect_uri} has not been registered.`)
                                }
                            }
            ...
            if (pathname === "/authorizeUrl") {
                ...
                console.log('Reminder: If this says "INVALID_CLIENT: Invalid redirect URI" you need to add the redirect URI to your Spotify app settings.')
                console.log(`App settings: https://developer.spotify.com/dashboard/${settings.client.id}/settings`)
            ...
            if (pathname === '/script.js') {
                res.writeHead(200, { 'Content-Type': 'application/javascript' }).end(getFile(SCRIPT_FILENAME) || '')
                return
            }
            if (pathname === '/') {
